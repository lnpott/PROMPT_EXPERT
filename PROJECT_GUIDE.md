...
#### PASSO 20.4 — APPLIED METHODOLOGY AUDIT — 2026-09-12

- **Base e escopo:** auditoria iniciada em `09e6180990776cbf2e52e429424739a712c2e90c`, após o merge do PR #36. Release ativa `2.1.0`; 23 targets públicos × 6 task types (`cited`, `application`, `refactor`, `debug`, `agent`, `fim`) = 138 resoluções auditadas sem chamar APIs pagas.
- **Ferramenta reproduzível:** `scripts/audit-methodology-resolution.mjs` aceita `--target`, `--task-type`, `--platform` e `--all`. A saída contém target/display/family/parent/version, path real, próprias/herdadas, prompt rules, anti-patterns, API/orchestration rules, rejeitadas com motivo, example, sources, provenance e prompt compilado.
- **Artefato:** `docs/methodology-applied-audit-2.1.0.md` registra todas as 138 combinações, texto integral e metadados das regras consideradas, separação prompt/API, exemplos, prompt efetivo por família, independência executor/target, paridade UI/runtime, gaps e bugs.
- **Resultado metodológico:** os 14 targets específicos têm zero regras próprias e `NO VERSION-SPECIFIC OVERRIDE`; herdam somente sua cadeia real. Famílias públicas usam suas regras próprias mais regras gerais. Não há active platform-specific prompt rule; a regra `PLATFORM` ativa é somente de orquestração. Targets desconhecidos falham fechados e corpus elegível vazio usa exclusivamente `safe-local-fallback`.
- **Examples e anti-patterns:** 46 das 138 combinações selecionam um dos 18 examples por ancestral mais próximo + task exato, apenas como referência estrutural. O briefing neutro não recebeu React, TypeScript, PostgreSQL ou Docker; briefing Python preservou Python. Anti-patterns são prompt rules explícitas; API_PARAMETER/API_CONSTRAINT/CACHE/PLATFORM permanecem no pacote auditável e fora do texto enviado ao compilador/gerador.
- **Production × fallback:** auditoria read-only confirmou payload remoto semanticamente idêntico ao snapshot local: 24 sources, 25 target nodes, 44 rules, 165 applicability, 18 examples e SHA-256 `f7b019502b7f418fc8d717bff23c79615fa4845afe600c30b42c7b7ffbf47e40`. Nenhuma escrita Production foi feita no Passo 20.4.
- **Bugs e correções:** (1) o motivo de conflito no trace era abreviado como `superseded`; foi normalizado para `superseded_by_higher_precedence`, com fixture version > family; (2) não existia visualização CLI completa/reproduzível do pacote aplicado; foi adicionado o script de auditoria e cobertura integral. Não houve mismatch UI/base, provenance failure, contaminação por API rule, regra de família incorreta ou example com stack.
- **Gaps preservados:** permanecem apenas os gaps de pesquisa já documentados no 2.1.0; esta etapa não promoveu nem pesquisou conteúdo novo. Auth, vault, crypto, credentials, secrets, providers, catálogo de geração, billing, lifecycle e Alibaba não foram alterados.

#### PASSO 20.5/20.6 — PRODUCT FLOW & REAL EXECUTION VALIDATION — 2026-09-12

- **Base:** `1673989e5938a1a9f3a49618b6762a5e5d938fd4`, já com PRs #36/#37. Branch `step-20-5-6-product-flow-real-execution`; release 2.1.0 e auditoria metodológica não foram alteradas.
- **UX:** a escolha “Compilação local determinística” agora antecede os controles, declara “Sem conta, sem chave e sem chamada externa. Não usa IA.” e oculta provider/credencial externos no modo local. Provider e modelo externo usam labels distintas; targets familiares recebem “Família”, sem expor o ancestral interno `claude-sonnet`.
- **Fluxos:** visitante conclui briefing → task → target → local → resultado → copy. BYOK deslogado mostra CTA de login sem perder briefing/target; logado sem chave vê “Configure sua chave”; status untested/valid/invalid/error permanecem distintos. Todos os 9 targets familiares × 6 tasks foram executados localmente.
- **Resultado e erros:** metadata separa executor, target e task. Fallback da Plataforma agora identifica “Compilador local (fallback)”, em vez de atribuir a resposta a Gemini. Falha de catálogo sai de loading permanente. Busy bloqueia também provider e origem da credencial; erros não limpam estado.
- **QA:** desktop Chrome 1440×1000 e mobile Chromium 390×844 foram inspecionados com fluxo browser mockado, geração, copy exato, metadata, bloqueio BYOK e preservação. Screenshots finais estão em `screenshots/step-20-5-6-{desktop,mobile}.png`.
- **Execução:** Supabase foi consultado read-only. Uma tentativa real controlada Gemini Platform com credencial já presente entrou no path correto, mas terminou em fallback local por `TypeError` do ambiente; não é declarada como sucesso real. Demais providers: `REAL_EXECUTION_NOT_RUN`, cobertos por mocks/contracts, sem consumo pago.
- **Segurança/escopo:** nenhuma migration/escrita Production, API paga deliberada, secret, Auth, vault, crypto, RLS, adapter, catálogo ou billing foi alterado. Alibaba continua inativo. O relatório completo, matrizes e limitações estão em `docs/product-flow-real-execution-audit.md`.
