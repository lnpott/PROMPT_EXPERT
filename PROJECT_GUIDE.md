...
- **Alibaba/Qwen:** [OpenAI compatibility](https://www.alibabacloud.com/help/en/model-studio/compatibility-of-openai-with-dashscope) e [Models](https://www.alibabacloud.com/help/en/model-studio/models) confirmam bearer, `/models`, `/chat/completions`, `qwen3.7-plus` e destinos regionais. Foi escolhido o host fixo US/Virginia `https://dashscope-us.aliyuncs.com/compatible-mode/v1`. A migration local `20260910120000_activate_alibaba_us_generation.sql` propõe ativação, URLs e aviso regional; **não foi aplicada remotamente** e requer autorização explícita. Até isso ocorrer, Qwen continua indisponível na UI remota.
