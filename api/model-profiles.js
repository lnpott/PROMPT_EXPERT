...
export const taskTypes = Object.freeze({ cited: 'Executar exatamente as tarefas citadas no briefing', application: 'Aplicação ou funcionalidade completa', refactor: 'Refatoração de código existente', debug: 'Diagnóstico e correção de problema', agent: 'Tarefa para agente autônomo de programação', fim: 'Completude de código Fill-in-the-Middle' });
...
export function selectMethodologyRules(profile, taskType = 'cited', modelRuleIds = []) {
  ...
  return engine(corpus).select(legacy, taskType).selected;
}
